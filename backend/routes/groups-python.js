...
router.get('/by-name/:groupName', async (req, res) => {
  ...
  const tripsWithSummary = trips.map(trip => {
    ...
    return {
      ...trip,
      summary: { adultCount, childCount, refundCount, ticketTotal, serviceFee, refundFee, verifyFee, total }
    };
  });
...
router.post('/import', upload.single('file'), async (req, res) => {
  try {
    ...
    const tempResult = parseExcel(req.file.path, 1000);
    ...
    const childPrice = Math.round(adultPrice * 0.5 * 100) / 100;
...
router.get('/:id/seats', async (req, res) => {
  ...
  console.log(`\n========================================`);
  console.log(`开始导出座位表 (Python版): 团ID=${id}, 团名=${group.name}`);
  ...
  try {
    ...
    if (!fs.existsSync(pythonScript)) {
      throw new Error('Python脚本不存在，请检查 tools/generate_seat_excel.py');
    }
